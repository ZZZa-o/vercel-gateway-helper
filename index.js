...
async function applyToCustomBody() {
    ...
    // 方法1: 动态导入 oai_settings 直接注入（只在点击时才加载，不影响扩展启动）
    try {
        const OpenAI = await import('../../../openai.js');
        if (OpenAI?.oai_settings) {
            OpenAI.oai_settings.custom_include_body = jsonStr;
    ...
    const textarea = document.querySelector('#custom_include_body')
        ...
        || [...document.querySelectorAll('textarea')].find(ta =>
            ta.id?.includes('include_body') || ta.name?.includes('include_body')
        );
    if (textarea) {
    ...
    navigator.clipboard.writeText(jsonStr).then(
        () => toast('已复制到剪贴板，请手动粘贴到 附加参数→包含主体参数', 'warning'),
        () => toast('写入失败', 'error'),
    );
}
...
function applyToCustomBody() {
    ...
    const textarea = document.querySelector('#custom_include_body')
        || document.querySelector('textarea[name="custom_include_body"]')
        || document.querySelector('#custom_include_body_openai')
        // 回退: 找所有 textarea，看哪个当前值像 JSON 且含 providerOptions
        || [...document.querySelectorAll('textarea')].find(ta =>
            ta.id?.includes('include_body') || ta.name?.includes('include_body')
        );
    if (textarea) {
...
jQuery(async () => {
    ...
    oai_settings: !!_OpenAI.oai_settings,
});
